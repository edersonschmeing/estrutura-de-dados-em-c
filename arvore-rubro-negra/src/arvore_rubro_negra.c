...
void destruir_no_ARN(No *ptr_no) { 

    if (ptr_no == NULL)
        return; 

    destruir_no_ARN(ptr_no->ptr_no_esquerda);
    destruir_no_ARN(ptr_no->ptr_no_direita);
    free(ptr_no);
    ptr_no = NULL;
...
void destruir_ARN(No **ptr_ptr_no_raiz) { 
    ...
    destruir_no_ARN(*ptr_ptr_no_raiz);
...
void imprime_ARN(No *ptr_no, int nivel) {
    ...
    imprime_ARN(ptr_no->ptr_no_direita, nivel + 1);
    imprime_no_ARN(ptr_no->chave, nivel);
    imprime_ARN(ptr_no->ptr_no_esquerda, nivel + 1);
